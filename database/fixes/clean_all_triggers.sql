...
DROP TRIGGER IF EXISTS trigger_sync_yahoo_scraped_to_master ON yahoo_scraped_products CASCADE;
DROP TRIGGER IF EXISTS trigger_sync_yahoo_to_master ON yahoo_scraped_products CASCADE;
DROP TRIGGER IF EXISTS trigger_dual_keys ON yahoo_scraped_products CASCADE;
DROP TRIGGER IF EXISTS trigger_clear_schedule_on_status_change ON yahoo_scraped_products CASCADE;

-- 3. 古い関数も削除
DROP FUNCTION IF EXISTS sync_yahoo_to_master() CASCADE;
DROP FUNCTION IF EXISTS trg_sync_yahoo_to_master() CASCADE;
DROP FUNCTION IF EXISTS sync_yahoo_scraped_products() CASCADE;

-- 4. 新しいトリガー関数を作成（descriptionなし）
