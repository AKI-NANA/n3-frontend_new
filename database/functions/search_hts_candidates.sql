-- database/functions/search_hts_candidates.sql
-- HTS候補検索ストアドファンクション（高精度FTS版）
-- 
-- PostgreSQL Full-Text Searchを使用した3段階スコアリング:
-- 1. フレーズ完全一致（最優先）
-- 2. 部分フレーズ一致
-- 3. 単語一致（フォールバック）
...
BEGIN
  ...
  SELECT 
    ...
    r.description_ja,
    r.general_rate_of_duty,
    r.special_rate_of_duty,
