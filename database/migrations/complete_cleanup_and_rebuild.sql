-- ============================================
-- 完全クリーンアップ + リアルタイム自動同期システム再構築
-- ============================================
-- STEP 1: 既存の全トリガーと関数を削除
-- STEP 2: 4テーブルの新規トリガーシステムを構築
-- ============================================
...
-- 既存関数の削除
DROP FUNCTION IF EXISTS sync_yahoo_to_products_master();
DROP FUNCTION IF EXISTS sync_inventory_to_products_master();
DROP FUNCTION IF EXISTS sync_ebay_to_products_master();
DROP FUNCTION IF EXISTS sync_research_to_products_master();
DROP FUNCTION IF EXISTS sync_mystical_to_products_master();

-- 念のため、類似名の関数も削除
DROP FUNCTION IF EXISTS sync_products_to_master() CASCADE;
DROP FUNCTION IF EXISTS update_products_master() CASCADE;

DO $$
BEGIN
    RAISE NOTICE '✅ 既存システムの削除完了';
END $$;
