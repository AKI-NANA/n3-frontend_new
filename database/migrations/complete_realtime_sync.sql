...
-- 完全版リアルタイム自動同期システム
-- ============================================
-- 全ソーステーブル → products_master の完全同期
...
-- ============================================
-- 3. mystical_japan_treasures_inventory 完全同期
-- ============================================

CREATE OR REPLACE FUNCTION sync_mystical_to_products_master()
RETURNS TRIGGER AS $$
BEGIN
    IF (TG_OP = 'INSERT') THEN
        INSERT INTO products_master (
        ...
        VALUES (
            ...
            NEW.profit_score,
            NEW.export_filter_pass,
            NEW.patent_filter_pass,
            NEW.mall_filter_pass,
            NEW.filter_issues,
            COALESCE(
    ...
    IF (TG_OP = 'UPDATE') THEN
        UPDATE products_master SET
            ...
            competition_score = NEW.competition_score,
            profit_score = NEW.profit_score,
            export_filter_pass = NEW.export_filter_pass,
            patent_filter_pass = NEW.patent_filter_pass,
            mall_filter_pass = NEW.mall_filter_pass,
            filter_issues = NEW.filter_issues,
            primary_image_url = COALESCE(
                NEW.primary_image_url,
                CASE WHEN NEW.images IS NOT NULL AND jsonb_array_length(NEW.images) > 0 
                     THEN NEW.images->>0 
            ...
            approval_status = COALESCE(NEW.approval_status, 'pending'),
            workflow_status = COALESCE(NEW.workflow_status, 'processed'),
            updated_at = NOW(),
            synced_at = NOW()
        WHERE source_system = 'mystical' 
        AND source_id = NEW.id::text;
        RETURN NEW;
    END IF;
...
CREATE TRIGGER trigger_sync_mystical_to_master
    AFTER INSERT OR UPDATE OR DELETE
    ON mystical_japan_treasures_inventory
    FOR EACH ROW
    EXECUTE FUNCTION sync_mystical_to_products_master();

-- ============================================
-- 4. ebay_inventory 完全同期（確認・補完）
-- ============================================
...
-- ============================================
-- 5. research_products_master 完全同期（確認・補完）
-- ============================================
...
BEGIN
    RAISE NOTICE '============================================';
    RAISE NOTICE '✅ 完全版リアルタイム自動同期システム構築完了';
    RAISE NOTICE '============================================';
    RAISE NOTICE '対象テーブル:';
    RAISE NOTICE '  - yahoo_scraped_products';
    RAISE NOTICE '  - inventory_master';
    RAISE NOTICE '  - mystical_japan_treasures_inventory';
