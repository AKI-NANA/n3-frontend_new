-- ============================================================
-- NAGANO-3 v2.0 完全統合データ同期システム - 修正版
-- Part 1: トリガー関数とマスター更新処理
-- ============================================================
-- 目的: products, yahoo_scraped_products, inventory_products テーブルを
--       products_master に完全同期し、双方向更新を実現
-- 
-- フィールド名修正:
--   - products.english_title (確認済み)
--   - yahoo_scraped_products.english_title
-- ============================================================
...
-- ============================================================
-- 5. inventory_products → products_master 同期トリガー関数
-- ============================================================
...
BEGIN
    -- title_en の決定
    v_title_en := get_title_en(NEW.title_en, NEW.title);
    
    -- profit 計算
    SELECT * INTO v_profit_amount, v_profit_margin 
    FROM calculate_profit_values(NEW.listing_price, NEW.cost_price);
    
    -- category の取得
    v_category := COALESCE(NEW.category, 'Uncategorized');
    ...
    IF TG_OP = 'INSERT' THEN
        INSERT INTO products_master (
            ...
            category, workflow_status, approval_status,
            listing_status, listing_price, inventory_quantity,
            ebay_item_id, ebay_listing_url, listed_at,
            created_at, updated_at
        ) VALUES (
            'inventory_products', NEW.id::TEXT, NEW.title, v_title_en,
            NEW.listing_price, COALESCE(NEW.cost_price, 0), v_profit_amount, v_profit_margin,
            v_category, 'in_inventory', 'approved',
            COALESCE(NEW.listing_status, 'not_listed'),
            NEW.listing_price, COALESCE(NEW.quantity_available, 0),
            NEW.ebay_item_id, NEW.ebay_listing_url, NEW.listed_at,
            NEW.created_at, NEW.updated_at
        )
        ON CONFLICT (source_system, source_id) DO NOTHING;
    ...
    ELSIF TG_OP = 'UPDATE' THEN
        UPDATE products_master SET
            title = NEW.title,
            title_en = v_title_en,
            current_price = NEW.listing_price,
            cost_price = COALESCE(NEW.cost_price, 0),
            profit_amount = v_profit_amount,
            profit_margin = v_profit_margin,
            category = v_category,
            listing_status = COALESCE(NEW.listing_status, 'not_listed'),
            listing_price = NEW.listing_price,
            inventory_quantity = COALESCE(NEW.quantity_available, 0),
            ebay_item_id = NEW.ebay_item_id,
            ebay_listing_url = NEW.ebay_listing_url,
            listed_at = NEW.listed_at,
            updated_at = NEW.updated_at
        WHERE source_system = 'inventory_products' AND source_id = NEW.id::TEXT;
        
    ELSIF TG_OP = 'DELETE' THEN
        DELETE FROM products_master 
        WHERE source_system = 'inventory_products' AND source_id = OLD.id::TEXT;
        RETURN OLD;
    END IF;
...
-- inventory_products テーブルのトリガー
DROP TRIGGER IF EXISTS trg_sync_inventory_to_master ON inventory_products;
CREATE TRIGGER trg_sync_inventory_to_master
...
BEGIN
    ...
    -- inventory_products テーブルへの逆同期
    ELSIF NEW.source_system = 'inventory_products' THEN
        UPDATE inventory_products SET
            title = NEW.title,
            title_en = NEW.title_en,
            listing_price = NEW.current_price,
            cost_price = NEW.cost_price,
            listing_status = NEW.listing_status,
            quantity_available = NEW.inventory_quantity,
            ebay_item_id = NEW.ebay_item_id,
            ebay_listing_url = NEW.ebay_listing_url,
            listed_at = NEW.listed_at,
            updated_at = NEW.updated_at
...
SELECT 
    'inventory_products' AS source_system,
    i.id::TEXT AS source_id,
    i.title,
    COALESCE(i.title_en, i.title) AS title_en,
    i.listing_price AS current_price,
    COALESCE(i.cost_price, 0) AS cost_price,
    (i.listing_price - COALESCE(i.cost_price, 0)) AS profit_amount,
    CASE 
        WHEN i.listing_price > 0 THEN ((i.listing_price - COALESCE(i.cost_price, 0)) / i.listing_price) * 100
        ELSE 0 
    END AS profit_margin,
    COALESCE(i.category, 'Uncategorized') AS category,
    'in_inventory' AS workflow_status,
    'approved' AS approval_status,
    COALESCE(i.listing_status, 'not_listed') AS listing_status,
    i.listing_price,
    COALESCE(i.quantity_available, 0) AS inventory_quantity,
    i.ebay_item_id,
    i.ebay_listing_url,
    i.listed_at,
    i.created_at,
...
BEGIN
    RAISE NOTICE '====================================';
    RAISE NOTICE 'Part 1 完了: トリガー関数とマスター更新処理';
    RAISE NOTICE '====================================';
    RAISE NOTICE '✓ ヘルパー関数作成完了';
    RAISE NOTICE '✓ 同期トリガー関数作成完了';
    RAISE NOTICE '✓ トリガー登録完了';
    RAISE NOTICE '✓ 逆方向同期設定完了';
    RAISE NOTICE '✓ 初回データ移行完了';
    RAISE NOTICE '';
    RAISE NOTICE '次: complete_sync_system_part2.sql を実行してください';
    RAISE NOTICE '====================================';
END $$;
