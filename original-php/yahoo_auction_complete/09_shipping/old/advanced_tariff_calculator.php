...
<body>
    ...
    <script>
        ...
        async function calculateEbayUSA() {
            ...
            try {
                const response = await fetch('advanced_tariff_api_fixed.php?action=ebay_usa_calculate', {
                    method: 'POST',
                ...
                if (result.success) {
                    displayResults(result.data, 'eBay USA');
                } else {
        ...
        async function calculateShopee7Countries() {
            ...
            try {
                const response = await fetch('advanced_tariff_api_fixed.php?action=shopee_7countries_calculate', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(formData)
                });
                ...
                const result = await response.json();
                if (result.success) {
                    displayResults(result.data, 'Shopee');
                } else {
                    alert('計算エラー: ' + result.error);
                }
            } catch (error) {
        ...
        function getEbayUSAFormData() {
            return {
                ...
                tariff_rates: {
                    electronics: parseFloat(document.getElementById('usa-electronics-tariff').value) || 7.5,
                    textiles: parseFloat(document.getElementById('usa-textiles-tariff').value) || 12.0,
                    other: parseFloat(document.getElementById('usa-other-tariff').value) || 5.0
                },
                additional_costs: {
                    outsource_fee: parseFloat(document.getElementById('usa-outsource-fee').value) || 0,
                    packaging_fee: parseFloat(document.getElementById('usa-packaging-fee').value) || 0,
                    exchange_margin: parseFloat(document.getElementById('usa-exchange-margin').value) || 5.0
                }
            };
        }
        ...
        function getShopee7CountriesFormData() {
            return {
                ...
                country_settings: countrySettings[selectedShopeeCountry]
            };
        }
        ...
        function loadShopeePreset() {
            ...
            // 設定保存機能から読み込み
            loadSettingsFromAPI('shopee');
        }

        async function saveEbayConfig() {
            const settings = {
                electronics_tariff: parseFloat(document.getElementById('usa-electronics-tariff').value) || 7.5,
                textiles_tariff: parseFloat(document.getElementById('usa-textiles-tariff').value) || 12.0,
                other_tariff: parseFloat(document.getElementById('usa-other-tariff').value) || 5.0,
                outsource_fee: parseFloat(document.getElementById('usa-outsource-fee').value) || 500,
                packaging_fee: parseFloat(document.getElementById('usa-packaging-fee').value) || 200,
                exchange_margin: parseFloat(document.getElementById('usa-exchange-margin').value) || 5.0,
                shipping_mode: currentShippingMode
            };
            ...
            try {
                ...
                if (result.success) {
                    ...
                    showNotification('設定保存エラー: ' + result.error, 'error');
                }
            } catch (error) {
                console.error('設定保存エラー:', error);
                showNotification('設定保存中にエラーが発生しました', 'error');
            }
        }

        async function saveShopeeConfig() {
            const settings = {
                selected_country: selectedShopeeCountry,
                default_outsource_fee: 300,
                default_packaging_fee: 150,
                default_exchange_margin: 3.0
            };
        ...
        async function loadSettingsFromAPI(category) {
            try {
                const response = await fetch(`tariff_settings_api.php?action=load_settings&category=${category}`);
                const result = await response.json();
                
                if (result.success && result.settings) {
                    const settings = result.settings;
                    
                    if (category === 'ebay_usa') {
                        if (settings.electronics_tariff) {
                            document.getElementById('usa-electronics-tariff').value = settings.electronics_tariff;
                        }
                        if (settings.textiles_tariff) {
                            document.getElementById('usa-textiles-tariff').value = settings.textiles_tariff;
                        }
                        if (settings.other_tariff) {
                            document.getElementById('usa-other-tariff').value = settings.other_tariff;
                        }
                        if (settings.outsource_fee) {
                            document.getElementById('usa-outsource-fee').value = settings.outsource_fee;
                        }
                        if (settings.packaging_fee) {
                            document.getElementById('usa-packaging-fee').value = settings.packaging_fee;
                        }
                        if (settings.exchange_margin) {
                            document.getElementById('usa-exchange-margin').value = settings.exchange_margin;
                        }
                        if (settings.shipping_mode) {
                            selectShippingMode(settings.shipping_mode);
                        }
                    } else if (category === 'shopee') {
                        if (settings.selected_country) {
                            selectShopeeCountry(settings.selected_country);
                        }
                    }
                    
                    showNotification('保存された設定を読み込みました', 'info');
                }
            } catch (error) {
        ...
        document.addEventListener('DOMContentLoaded', async function() {
            ...
            // 保存された設定を自動読み込み
            try {
                await loadSettingsFromAPI('ebay_usa');
                await loadSettingsFromAPI('shopee');
                console.log('✅ 保存された設定の自動読み込み完了');
            } catch (error) {
                console.log('ℹ️ 設定ファイル未作成 - デフォルト値を使用');
            }
            
            console.log('高度統合利益計算システム初期化完了 - 設定保存機能付き');
        });
    </script>
</body>
</html>