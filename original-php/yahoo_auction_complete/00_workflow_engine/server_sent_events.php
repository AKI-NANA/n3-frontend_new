<?php
/**
 * Server-Sent Events システム
 * リアルタイム監視・ダッシュボード用ストリーミング
 */
...
class WorkflowSSE {
    ...
    private function getQueueStats() {
        ...
        try {
            ...
            $stats['fallback_mode'] = ($this->redis === null);
     ...
     * アラート送信
     */
    public function sendAlert($message, $severity = 'info') {
        $this->sendEvent('alert', [
            'message' => $message,
            'severity' => $severity,
            'timestamp' => time()
        ]);
     ...
     * ワークフロー更新通知
     */
    public function sendWorkflowUpdate($workflowId, $status, $progress) {
        $this->sendEvent('workflow_update', [
    ...
    private function initRedis() {
        try {
            ...
            // Redis接続失敗は非致命的エラー
            error_log('Redis接続失敗: ' . $e->getMessage());
        }
        return null;
    }
    ...
    private function getDatabaseConnection() {
        try {
            ...
            error_log("SSE: データベース接続エラー: " . $e->getMessage());
            return null;
        }
    }
}
