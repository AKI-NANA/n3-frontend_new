...
function getAllProductsData($page = 1, $limit = 20, $filters = []) {
    ...
    try {
        ...
        $sql = "SELECT 
                    ...
                    price_jpy as price,
                    active_price_usd as current_price,
                    ...
                    current_stock,
                    ebay_item_id,
                    COALESCE(category, 'N/A') as category_backup
                FROM {$actualTable} 
                {$whereClause} 
...
function getScrapedProductsData($page = 1, $limit = 20, $filters = []) {
    ...
    try {
        ...
        // 未出品データのみ取得（ebay_item_idがNULLまたは空、かつ全データも含む場合）
        // デバッグ用に一時的に条件を緩和
        ...
        // データ取得クエリ（完全版・Yahoo Auctionスクレイピングデータ用）
        $sql = "SELECT 
                    id,
                    source_item_id as item_id,
                    COALESCE(active_title, 'タイトルなし') as title,
...
<head>
    ...
    * { box-sizing: border-box; }
    ...
    .source-badge.source-yahoo { background: var(--accent-navy); }
    .source-badge.source-ebay { background: var(--accent-purple); }
    .source-badge.source-inventory { background: var(--accent-lightblue); color: var(--text-primary); }
    .source-badge.source-mystical { background: var(--accent-cream); color: var(--text-primary); }
