        ...
        let selectedEbayImages = [];
        
        // 既存のJavaScriptに統合モーダル機能を追加
        function openIntegratedModal(productId) {
            const modal = document.getElementById('integrated-modal');
            modal.style.display = 'block';
            document.body.style.overflow = 'hidden';
            
            try {
                loadIntegratedProductData(productId);
            } catch (error) {
        ...
        async function loadIntegratedProductData(productId) {
            try {
                ...
                if (result.success) {
                    ...
                    const title = result.data.basic_info.product.title || 'タイトルなし';
                    ...
                    addLog(`統合データ読み込み完了: ${title}`, 'success');
                } else {
