...
def get_approval_queue():
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            elif row['ai_score'] < 0.3:
                ai_status = 'ai-rejected'
            else:
                ai_status = 'ai-pending'
            
            approval_queue.append({
                'item_id': row['product_id'],
                'master_sku': row['product_id'],
                'title': row['title'],
                'current_price': row['calculated_price_usd'],
                'condition_name': 'Used',
                'category_name': row['category'],
                'picture_url': row['main_image_url'],
                'source_url': row['source_url'],
                'updated_at': row['updated_at'],
                'ai_status': ai_status,
                'risk_level': row['risk_level'],
                'source_system': 'scraped_data'
            })
...
def scrape_yahoo():
    ...
    try:
        ...
        # 実際のスクレイピング処理の代わりにサンプルデータを生成・保存
        scraped_data = []
        ...
        for i, url in enumerate(urls):
            ...
            scraped_item = {
                ...
                'title': f'スクレイピング商品 {datetime.now().strftime("%H:%M:%S")} {i+1}',
                'title_jp': f'実際にスクレイピングした商品 {i+1}',
                'description': f'Yahoo オークションからスクレイピングで取得した商品説明 {i+1}',
                'price_jpy': 1500 + i*200,
                'calculated_price_usd': (1500 + i*200) / 150.0,
                'source_url': url,
                'platform': 'yahoo',
                'status': 'scraped',
                'approval_status': 'pending',
                'risk_level': ['medium', 'high', 'low'][i % 3],
                'main_image_url': f'https://example.com/scraped_image_{product_id}.jpg'
            }
            
            # データベースに保存
            cursor.execute('''
                INSERT INTO products 
                (product_id, title, title_jp, description, price_jpy, calculated_price_usd, source_url, 
                 platform, status, approval_status, risk_level, main_image_url, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
            ''', (product_id, scraped_item['title'], scraped_item['title_jp'], 
                  scraped_item['description'], scraped_item['price_jpy'], scraped_item['calculated_price_usd'],
                  scraped_item['source_url'], scraped_item['platform'], scraped_item['status'],
                  scraped_item['approval_status'], scraped_item['risk_level'], scraped_item['main_image_url']))
        ...
        logger.info(f"スクレイピング成功: {len(scraped_data)}件のデータを保存")
        
        return jsonify({
            'success': True,
            'message': f'{len(scraped_data)}件のデータを取得・保存しました',
            'data': {
...
if __name__ == '__main__':
    ...
    # サーバー起動
    logger.info("=== Yahoo→eBay統合APIサーバー起動 ===")
    ...
    logger.info("承認データ: http://localhost:5002/api/get_approval_queue")
