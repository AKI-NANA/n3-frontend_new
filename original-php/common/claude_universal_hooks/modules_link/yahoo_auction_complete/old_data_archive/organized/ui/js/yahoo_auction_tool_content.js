/**
 * Yahoo Auction Tool - 実スクレイピング機能修正版JavaScript
 * APIエンドポイント修正・実データ対応版
 * 2025-09-12 修正: APIサーバー（localhost:5002）正常接続
 */
...
function loadApprovalData() {
    ...
    SystemLogger.info('実スクレイピングAPIサーバーから承認データ読み込み開始');
    ...
    // 修正: 実APIサーバーエンドポイント
    fetch(API_BASE_URL + '/api/get_approval_queue')
        .then(response => {
            if (!response.ok) {
        ...
        .then(data => {
            ...
            if (data.success && data.data) {
                ...
                SystemLogger.success(`実スクレイピングAPIから実データ取得成功: ${dataCount}件の商品データを表示`);
                ...
                container.innerHTML = `
                    <div class="notification error">
                        ...
                        <span>実スクレイピングAPIエラー: データを取得できませんでした</span>
                    </div>
                `;
            }
        })
        .catch(error => {
            ...
            systemStats.apiConnected = false;
            SystemLogger.error(`実スクレイピングAPI接続失敗: ${error.message}`);
            
            container.innerHTML = `
                <div class="notification error">
                    <i class="fas fa-exclamation-triangle"></i>
                    <span>実スクレイピングAPIサーバー（localhost:5002）に接続できません: ${error.message}</span>
                </div>
            `;
        })
        .finally(() => {
...
function displayApprovalProducts(products) {
    ...
    if (!products || products.length === 0) {
        container.innerHTML = `
            <div class="notification info">
                ...
                <span>承認待ちの商品がありません（実スクレイピングAPI確認済み）</span>
            </div>
        `;
        return;
    }
    ...
    // スクレイピングデータと既存データを分類
    const scrapedData = products.filter(p => p.source_system === 'yahoo_scraped_confirmed' || (p.source_url && p.source_url.includes('auctions.yahoo.co.jp')));
    const existingData = products.filter(p => !p.source_url || p.source_url === '');
    
    SystemLogger.info(`実データ分類: Yahooスクレイピング${scrapedData.length}件、既存${existingData.length}件`);
    ...
    const productsHtml = products.map((product, index) => {
        ...
        // 実Yahooスクレイピングデータかどうかの判定
        const isRealScrapedData = product.source_system === 'yahoo_scraped_confirmed' || (product.source_url && product.source_url.includes('auctions.yahoo.co.jp'));
        const dataTypeIcon = isRealScrapedData ? '🕷️' : '💾';
        const dataTypeLabel = isRealScrapedData ? 'Yahoo実取得' : '既存データ';
        
        return `
            <div class="approval-product-card" data-sku="${itemId}" data-scraped="${isRealScrapedData}">
                ...
                <div class="product-info">
                    ...
                    ${isRealScrapedData ? `<div class="scraped-url"><small>🔗 ${product.source_url ? product.source_url.substring(0, 50) + '...' : 'Yahoo取得済み'}</small></div>` : ''}
                </div>
    ...
    SystemLogger.success(`${products.length}件の商品を表示完了（Yahoo実スクレイピング: ${scrapedData.length}件）`);
}
...
function loadEditingData(page = 1, debug = false) {
    ...
    // 修正: 実APIサーバーのスクレイピングデータ取得（パラメータ修正）
...
document.addEventListener('DOMContentLoaded', function() {
    SystemLogger.success('システム初期化完了（実スクレイピング機能修正版）');
    SystemLogger.info('実スクレイピングAPIサーバー: localhost:5002 に接続設定済み');
    ...
    fetch(PHP_BASE_URL + '?action=get_dashboard_stats')
        ...
        .then(data => {
            if (data.success && data.data) {
                ...
                SystemLogger.info(`ダッシュボード統計更新完了（実スクレイピング: ${stats.scraped_count}件）`);
            }
        })
        .catch(error => {
