-- Yahoo Auction Tool 送料・利益計算システム完全版データベース
-- 過去の決定事項を全て反映

-- 1. 送料サービスマスター
CREATE TABLE IF NOT EXISTS shipping_services (
    service_id SERIAL PRIMARY KEY,
    carrier_name VARCHAR(100) NOT NULL, -- 'eLogi', 'cpass', '日本郵便', 'FedEx', 'DHL'
...
-- 5. 商品マスター拡張
CREATE TABLE IF NOT EXISTS item_master_extended (
...
-- 6. カテゴリー別重量推定データ
CREATE TABLE IF NOT EXISTS category_weight_estimation (
...
-- 7. 為替レートキャッシュ拡張
CREATE TABLE IF NOT EXISTS exchange_rates_extended (
    rate_id SERIAL PRIMARY KEY,
...
-- 8. 利益計算履歴
CREATE TABLE IF NOT EXISTS profit_calculation_history (
    calculation_id SERIAL PRIMARY KEY,
    item_code VARCHAR(100) REFERENCES item_master_extended(item_code),
...
-- サンプル送料サービス投入
INSERT INTO shipping_services (carrier_name, service_name, service_code, max_weight_kg, max_length_cm, max_width_cm, max_height_cm, max_girth_cm, estimated_delivery_days_min, estimated_delivery_days_max) VALUES
...
SELECT s.service_id, s.carrier_name, s.service_name, s.service_code,
