-- 🔥 Yahoo スクレイピング + eBay API 統合データベース（107項目以上対応）
-- スクレイピングシステムベース + eBay API完全統合版
...
CREATE TABLE unified_scraped_ebay_products (
    ...
    product_id VARCHAR(20) UNIQUE NOT NULL, -- スクレイピング由来のUUID短縮
    master_sku VARCHAR(255), -- 手動設定可能なマスターSKU
    
    -- 🎯 重複防止ハッシュシステム
    title_hash VARCHAR(32), -- タイトルハッシュ（重複検出用）
    price_range_hash VARCHAR(16), -- 価格帯ハッシュ
    duplicate_detection_hash VARCHAR(64), -- 複合ハッシュ（タイトル+価格+画像）
    
    -- 📊 データソース管理
    data_source_priority VARCHAR(20) DEFAULT 'scraped', -- 'scraped', 'ebay_api', 'manual'
    integration_status VARCHAR(20) DEFAULT 'scraped', -- 'scraped', 'integrated', 'conflicted'
    ...
    yahoo_url TEXT,
    yahoo_auction_id VARCHAR(50), -- Yahoo商品ID（URL抽出）
    ...
    -- 画像情報（スクレイピング）
    scraped_image_urls TEXT, -- |区切りURL文字列
    ...
    status VARCHAR(20) DEFAULT 'scraped', -- scraped, edited, listed, sold_out, error
    ...
    -- 表示用統合データ（最優先データから自動選択）
    active_title TEXT, -- 日本語 or 英語タイトル
    active_description TEXT, -- 日本語 or 英語説明
    active_price_usd DECIMAL(12,2), -- 統一USD価格
    active_price_jpy INTEGER, -- 統一円価格
    active_image_url TEXT, -- メイン画像URL
    active_image_urls JSONB, -- 統合画像URL配列
    active_category VARCHAR(255), -- 統合カテゴリ
    active_condition VARCHAR(50), -- 統合商品状態
    ...
    -- パフォーマンス統合
    total_views INTEGER DEFAULT 0, -- スクレイピング + eBay合計
...
CREATE TABLE product_editing_history (
    ...
    old_value TEXT,
    new_value TEXT,
    edit_type VARCHAR(20), -- 'manual', 'auto_translation', 'api_update'
...
BEGIN
    ...
    SELECT COUNT(*) INTO total_views FROM information_schema.views WHERE table_schema = 'public' AND table_name LIKE '%_report' OR table_name LIKE '%_summary';
