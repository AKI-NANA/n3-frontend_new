...
<head>
    ...
    <link href="css/yahoo_auction_tool_styles_complete.css" rel="stylesheet">
</head>
...
if ($bodyContent === false) {
    // ファイルが見つからない場合は inline で表示
    include __DIR__ . '/html/yahoo_auction_tool_body_inline.php';
} else {
    // 動的データを置換
    $bodyContent = str_replace(
        ...
        [
            number_format($dashboard_stats['total_records'] ?? 0),
            number_format($dashboard_stats['scraped_count'] ?? 0),
            number_format($dashboard_stats['calculated_count'] ?? 0),
            number_format($dashboard_stats['filtered_count'] ?? 0),
            number_format($dashboard_stats['ready_count'] ?? 0),
            number_format($dashboard_stats['listed_count'] ?? 0),
            $_SESSION['csrf_token']
        ]
    );
    echo $bodyContent;
