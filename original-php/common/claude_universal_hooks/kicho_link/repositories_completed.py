...
class JournalEntryRepository(BaseRepository[JournalEntry, str]):
    ...
    async def find_by_transaction_id(self, transaction_id: str) -> List[JournalEntry]:
        """トランザクションIDによる仕訳データ検索
        ...
        Returns:
            仕訳データのリスト
        """
        query = select(JournalEntry).where(JournalEntry.transaction_id == transaction_id)
        result = await self.session.execute(query)
        return result.scalars().all()
...
class ActivityLogRepository(BaseRepository[ActivityLog, str]):
    ...
    async def find_recent_activities(self, limit: int = 20) -> List[ActivityLog]:
        """最近のアクティビティログを検索
    ...
    async def find_by_user(self, user: str, limit: int = 100) -> List[ActivityLog]:
        """ユーザーによるアクティビティログ検索
    ...
    async def find_by_date_range(self, 
                                start_date: datetime, 
                                end_date: datetime,
                                activity_type: Optional[str] = None) -> List[ActivityLog]:
        """日付範囲によるアクティビティログ検索
        
        Args:
            start_date: 開始日時
            end_date: 終了日時
            activity_type: アクティビティタイプ（Noneの場合は全て）
        ...
        query = select(ActivityLog).where(and_(*conditions)).order_by(ActivityLog.created_at.desc())
        result = await self.session.execute(query)
        return result.scalars().all()
    ...
    async def log_activity(self, 
                         ...
                         data: Optional[Dict[str, Any]] = None) -> ActivityLog:
        """アクティビティをログに記録
...
# ファクトリー関数
def get_rule_repository(session: AsyncSession) -> RuleRepository:
    """ルールリポジトリを取得
...
def get_journal_entry_repository(session: AsyncSession) -> JournalEntryRepository:
    """仕訳データリポジトリを取得
...
def get_correction_repository(session: AsyncSession) -> CorrectionRepository:
    """修正履歴リポジトリを取得
    ...
    Returns:
        修正履歴リポジトリ
    """
    return CorrectionRepository(session)

def get_activity_log_repository(session: AsyncSession) -> ActivityLogRepository:
    """アクティビティログリポジトリを取得
