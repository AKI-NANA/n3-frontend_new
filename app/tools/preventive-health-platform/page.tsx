...
// --- 環境変数とFirebaseの初期化 ---
const appId = typeof __app_id !== 'undefined' ? __app_id : 'health-pro-v2-default';
...
// --- ダミーデータ: 栄養目標と高度なレシピデータ ---
const DAILY_GOALS = { protein: 120, iron: 10, calcium: 600, sodium: 1500, sugar: 50 }; // sodium/sugarは制限栄養素の例
const NUTRIENT_UNITS = { protein: 'g', iron: 'mg', calcium: 'mg', sodium: 'mg', sugar: 'g' };

// ユーザーが避けるべき食品/成分のリスト (Firestoreに保存されることを想定)
const USER_PROHIBITIONS = [
  { type: 'additive', value: '着色料' },
  { type: 'nutrient', value: 'sugar', threshold: 40 }, // 1食あたり砂糖40g以上は注意
  { type: 'food', value: '輸入牛肉' },
];

const DUMMY_RECIPES = [
  { 
    id: 'r1', name: '高タンパク鶏むね肉グリル (国産)', nutrients: { protein: 45, iron: 2, calcium: 50, sodium: 300, sugar: 5 }, isLunch: true,
    advParams: { preparationType: '加熱', origin: '国産', hasAdditives: 'なし', foodPairing: 'ブロッコリー' } 
  },
  { 
    id: 'r2', name: '鉄分たっぷりほうれん草ソテー (農薬あり)', nutrients: { protein: 10, iron: 5, calcium: 100, sodium: 500, sugar: 10 }, isDinner: true,
    advParams: { preparationType: '加熱', origin: '国産', hasAdditives: 'なし', foodPairing: 'レバー' } 
  },
  { 
    id: 'r3', name: '輸入フルーツの砂糖漬けヨーグルト', nutrients: { protein: 15, iron: 0.5, calcium: 250, sodium: 50, sugar: 65 }, isBreakfast: true, // 砂糖が高いため警告対象
    advParams: { preparationType: '冷用', origin: '輸入', hasAdditives: '着色料', foodPairing: 'なし' } // 着色料のため警告対象
  },
  { 
    id: 'r4', name: '有機野菜とツナのサラダ', nutrients: { protein: 20, iron: 1, calcium: 30, sodium: 200, sugar: 5 }, isLunch: true,
    advParams: { preparationType: '生', origin: '有機', hasAdditives: 'なし', foodPairing: '穀物' } 
  },
];

const HEALTH_RISKS = [
  { name: '高血圧リスク', targetNutrient: 'sodium', prevention: '減塩、有酸素運動', medical: '血圧・ナトリウム値検査' },
  { name: '貧血リスク', targetNutrient: 'iron', prevention: 'ツボ(足三里)、鉄分摂取', medical: 'フェリチン・ヘモグロビン検査' },
  { name: '高血糖リスク', targetNutrient: 'sugar', prevention: '食後15分歩行、クロム摂取', medical: 'HbA1c、血糖値検査' },
];
...
// --- UIコンポーネント: 献立実績タスクチェックボックス ---
const IntakeCheckbox = ({ meal, onToggle }) => (
  <div className="flex items-center space-x-2">
    <label className="flex items-center cursor-pointer">
      ...
      />
      <span className={`ml-2 text-sm ${meal.isCompleted ? 'line-through text-gray-400' : 'text-gray-700'}`}>摂取完了</span>
    </label>
  </div>
);
...
const PreventionView = () => (
    <div className="p-8 bg-white rounded-xl shadow-inner min-h-[70vh]">
                ...
                <div key={index} className="p-5 border border-red-300 bg-red-50 rounded-lg shadow-md hover:shadow-xl transition duration-300">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
                        <div className="bg-yellow-100 p-3 rounded-md">
                            <p className="font-semibold text-gray-800 border-b pb-1 mb-1">栄養による予防</p>
                            <p className="text-gray-600">制限/強化栄養素: **{risk.targetNutrient}** を厳しく管理します。</p>
                        </div>
                        <div className="bg-blue-100 p-3 rounded-md">
                            <p className="font-semibold text-gray-800 border-b pb-1 mb-1">運動/生活対策</p>
                            <p className="text-gray-600">実行すべき対策: **{risk.prevention}**</p>
                        </div>
                        <div className="bg-green-100 p-3 rounded-md">
                            <p className="font-semibold text-gray-800 border-b pb-1 mb-1">医療対策</p>
        ...
        <div className="mt-10 p-6 bg-gray-100 rounded-xl border border-gray-300">
            <h3 className="text-xl font-bold text-gray-800 mb-4">
                将来の高度なデータ蓄積システム (AI画像解析)
            </h3>
            <p className="text-sm text-gray-600 mb-3">
                このプラットフォームでは、食品の画像解析により、以下の高度なパラメーターをデータベースに蓄積し、より精密なアドバイスを提供することを目指しています。
            </p>
            <ul className="list-disc list-inside text-sm text-gray-700 grid grid-cols-2 gap-1">
                <li>食べ合わせ</li>
                <li>食べる時間帯、食事時間</li>
                <li>摂取の仕方（加熱、生、冷用など）</li>
                <li>有機栽培か農薬ありか</li>
                <li>輸入か国産か</li>
                <li>添加物、化学薬品の使用有無</li>
            </ul>
...
const App = () => {
  ...
  const [showAlert, setShowAlert] = useState(null); // 警告メッセージ
  ...
  const checkProhibitions = useCallback((recipe) => {
    ...
    USER_PROHIBITIONS.forEach(prohibition => {
      if (prohibition.type === 'additive' && recipe.advParams.hasAdditives === prohibition.value) {
        warning = `${recipe.name} には避けるべき添加物 (${prohibition.value}) が含まれています。`;
      } else if (prohibition.type === 'food' && recipe.name.includes(prohibition.value)) {
        warning = `${recipe.name} は、あなたが避けるべき食品 (${prohibition.value}) を含みます。`;
      } else if (prohibition.type === 'nutrient' && recipe.nutrients[prohibition.value] > prohibition.threshold) {
        warning = `${recipe.name} は${prohibition.value}が${prohibition.threshold}${NUTRIENT_UNITS[prohibition.value]}を超えています。`;
      }
    });
  ...
  // 献立の追加
  const handleAddMeal = useCallback(async (dateStr, recipeId, isPlanned = true) => {
    if (!userId || !db) return;
    ...
    const newMeal = {
      ...
      advParams: recipe.advParams, // 高度なパラメーターも保存
      isPlanned: isPlanned,
      isCompleted: false, 
      timestamp: Date.now(),
    };
  ...
  // 摂取実績のトグルと食後対策の提案
  const handleToggleIntake = useCallback(async (mealId) => {
    if (!userId || !db) return;
    const meal = menuPlans.find(m => m.id === mealId);
    if (!meal) return;
    
    const mealRef = doc(db, 'artifacts', appId, 'users', userId, 'menu_plans', mealId);
    try {
      await updateDoc(mealRef, { isCompleted: !meal.isCompleted });
      
      // 摂取完了時に食後対策を促す（アラートではないメッセージボックス）
      if (!meal.isCompleted) {
        console.log(`摂取完了！食後の対策が必要です。${meal.recipeName}に含まれる糖質対策のため、15分間の軽めの運動を実行しましょう。`);
      }
  ...
  // 特定日の栄養摂取量の計算 (実績のみ)
  const calculateDailyIntake = useCallback((dateStr) => {
    const dailyMeals = menuPlans.filter(p => p.date === dateStr && p.isCompleted);
    const intake = {};
  ...
  // 目標達成度の計算
  const checkGoalAchievement = useCallback((date) => {
  ...
  // 翌日の献立調整ロジック（モック）
  const handleAdjustNextDay = useCallback(async (date) => {
    ...
    Object.keys(deficiencies).forEach(nut => {
      if (deficiencies[nut] > (DAILY_GOALS[nut] * 0.2)) { 
        const recommendedRecipe = DUMMY_RECIPES.find(r => r.nutrients[nut] > 30); 
        if (recommendedRecipe) {
          handleAddMeal(nextDayStr, recommendedRecipe.id, true); 
          adjustments++;
        }
      }
    });
    ...
    if (adjustments > 0) {
      alert(`${formatDate(yesterday)}の不足に基づき、${nextDayStr}の献立に${adjustments}件の調整案を追加しました。`);
    } else {
  ...
  const GoalBadge = ({ date }) => {
    ...
    if (allAchieved) {
      return <span className="text-xs font-bold px-2 py-0.5 rounded-full bg-green-100 text-green-700">✅ 達成</span>;
    }
    ...
    if (Object.keys(deficiencies).length > 0) {
        return (
            <span 
                ...
                title={`不足: ${Object.keys(deficiencies).map(n => `${n}:${deficiencies[n].toFixed(0)}${NUTRIENT_UNITS[n]}`).join(', ')}`}
            >
                ⚠️ 不足
            </span>
        );
        ...
        <div className="flex justify-between items-center mb-6 border-b pb-3">
            <h2 className="text-2xl font-bold text-gray-800">
              {calendarView === 'month' 
                ...
                : currentDate.toLocaleDateString('ja-JP', { year: 'numeric', month: 'short', day: 'numeric' }) + 'の週'
              }
            </h2>
            
            <div className="flex space-x-3 items-center">
                <button 
                    className="bg-indigo-500 text-white font-bold py-2 px-4 rounded-lg hover:bg-indigo-600 transition shadow-md"
                    onClick={() => console.log("Googleカレンダー同期モック: 献立予定を同期するAPIを呼び出します")}
                >
                ...
                >
                    {calendarView === 'month' ? '週表示に切り替え' : '月表示に切り替え'}
                </button>
          ...
          <div className={`grid ${calendarView === 'month' ? 'grid-cols-7' : 'grid-cols-7'}`}>
            ...
            {calendarDays.map((date, index) => {
              ...
              const dayMeals = menuPlans.filter(m => m.date === dateStr);
              ...
              return (
                <div 
                  ...
                  className={`
                    ...
                    ${isCurrentMonth || calendarView === 'week' ? 'bg-white' : 'bg-gray-50 text-gray-400'}
                ...
                >
                  <div className="flex justify-between items-start mb-1">
                      ...
                      <div className="flex flex-col items-end space-y-1">
                        <GoalBadge date={date} />
                        {/* 翌日調整ボタン (当日の翌日に表示) */}
                        {formatDate(date) === formatDate(new Date(new Date().getTime() + 86400000)) && (
                            <button
                  ...
                  <div className="space-y-1">
                    {dayMeals.map(meal => (
                      <div key={meal.id} className="bg-gray-100 p-2 rounded-lg shadow-sm border border-gray-200">
                        <div className="flex justify-between items-start">
                            <p className="text-sm font-semibold text-gray-800 leading-tight">{meal.recipeName}</p>
                            <button 
                        ...
                        <div className="flex justify-between items-center mt-1">
                            <IntakeCheckbox meal={meal} onToggle={handleToggleIntake} />
                            {meal.isCompleted && (
                                <button
                                    onClick={() => console.log(`食後対策: 摂取した${meal.recipeName}（${meal.advParams.hasAdditives === '着色料' ? '注意食品' : '通常'}）の対策として食後運動を実行します`)}
                  ...
                  <div className="mt-2 absolute bottom-2 left-2 right-2">
                    <select 
                        onChange={(e) => handleAddMeal(dateStr, e.target.value)}
                        className="text-xs p-1 border-none bg-indigo-100 rounded w-full cursor-pointer text-indigo-700 font-medium"
                        value=""
                    >
                        <option value="" disabled>+ 献立を追加...</option>
                        {DUMMY_RECIPES.map(r => <option key={r.id} value={r.id}>{r.name}</option>)}
                    </select>
                  </div>
                </div>
              );
  ...
  // --- UI レンダリング ---
  const [currentPage, setCurrentPage] = useState('calendar'); // 'calendar' or 'prevention'

  if (!isAuthReady) {
    return <div className="flex items-center justify-center min-h-screen bg-gray-100"><div className="text-xl text-indigo-600">認証中...</div></div>;
  }
    ...
    <div className="min-h-screen bg-gray-50 font-sans">
      ...
      <header className="bg-white shadow-md">
        <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex justify-between items-center">
          <h1 className="text-2xl font-extrabold text-indigo-700">パーソナル予防医療プラットフォーム</h1>
          ...
          <div className="text-sm text-gray-500">
            ユーザーID: <span className="font-mono text-xs bg-gray-100 p-1 rounded">{userId}</span>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-screen-2xl mx-auto p-4 lg:p-8">
        <div className="bg-white rounded-xl shadow-2xl overflow-hidden">
          {currentPage === 'calendar' ? <CalendarView /> : <PreventionView />}
        </div>
      </main>

      {/* フッター */}
      <footer className="py-4 text-center text-xs text-gray-400">
        ※本システムは、将来的なAI画像解析と高度なデータ蓄積を想定したUI/UXです。データはFirestoreに保存されています。
      </footer>
    </div>
  );
};
