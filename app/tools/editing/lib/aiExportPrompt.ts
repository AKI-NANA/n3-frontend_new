// app/tools/editing/lib/aiExportPrompt.ts
/**
 * AI解析用プロンプト生成ユーティリティ
 * HTS自動判定結果を含めた市場調査データ取得
 */

interface ProductForAI {
  id: string
  sku: string
  title: string
  title_en?: string
  english_title?: string
  price_jpy: number
  msrp?: number
  release_date?: string
  category_name?: string
  category_id?: string
  length_cm?: number
  width_cm?: number
  height_cm?: number
  weight_g?: number
  condition?: string
  image_url?: string
  brand?: string
  hts_code?: string
  hts_confidence?: string
  origin_country?: string
  material?: string
  ebay_api_data?: any
}

interface HTSAutoResult {
  productId: string
  htsCode?: string
  confidence?: string
  matchedKeywords?: string[]
  source?: string
  error?: string
}

export async function generateAIAnalysisPrompt(
  products: ProductForAI[],
  autoHTSResults?: HTSAutoResult[]
): Promise<string> {
  // CSV生成
  const headers = [
    'SKU',
    '商品ID', 
    '商品名(日本語)',
    '英語タイトル',
    '仕入価格(円)',
    '定価(円)',
    '発売日',
    'カテゴリ名',
    'カテゴリID',
    '長さ(cm)',
    '幅(cm)',
    '高さ(cm)',
    '重さ(g)',
    '状態',
    '画像URL',
    'ブランド',
    '自動判定HTS',
    'HTS信頼度',
    '原産国'
  ]
  
  const csvRows = [headers.join(',')]
  
  products.forEach(p => {
    // 自動判定結果を取得
    const autoResult = autoHTSResults?.find(r => r.productId === p.id)
    
    const row = [
      p.sku || '',
      p.id || '',
      `"${(p.title || '').replace(/"/g, '""')}"`,
      `"${(p.title_en || p.english_title || '').replace(/"/g, '""')}"`,
      p.price_jpy || '',
      p.msrp || p.price_jpy || '',
      p.release_date || '不明',
      `"${(p.category_name || '').replace(/"/g, '""')}"`,
      p.category_id || '',
      p.length_cm || '',
      p.width_cm || '',
      p.height_cm || '',
      p.weight_g || '',
      `"${(p.condition || '').replace(/"/g, '""')}"`,
      `"${(p.image_url || '').replace(/"/g, '""')}"`,
      `"${(p.brand || '').replace(/"/g, '""')}"`,
      autoResult?.htsCode || p.hts_code || '要確認',
      autoResult?.confidence || p.hts_confidence || 'uncertain',
      p.origin_country || ''
    ]
    csvRows.push(row.join(','))
  })
  
  const csvContent = csvRows.join('\n')
  
  // HTS自動判定の失敗商品リスト
  const uncertainHTSProducts = products.filter((p, idx) => {
    const autoResult = autoHTSResults?.find(r => r.productId === p.id)
    const htsCode = autoResult?.htsCode || p.hts_code
    const confidence = autoResult?.confidence || p.hts_confidence
    return !htsCode || htsCode === '要確認' || confidence === 'uncertain' || confidence === 'low'
  })

  return `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🤖 AI商品分析 - 市場調査 + HTS判定 + 基本情報取得
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

📦 対象商品: ${products.length}件
${uncertainHTSProducts.length > 0 ? `⚠️ HTS要確認商品: ${uncertainHTSProducts.length}件` : '✅ 全商品のHTSコード自動判定済み'}

以下の商品データを**慎重に**分析してください：

${csvContent}

${uncertainHTSProducts.length > 0 ? `
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
⚠️ HTS自動判定失敗リスト（要手動判定）
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

以下の${uncertainHTSProducts.length}商品は自動判定に失敗しました。
各商品の特徴を分析し、正しいHTSコードを判定してください：

${uncertainHTSProducts.map((p, idx) => {
  const autoResult = autoHTSResults?.find(r => r.productId === p.id)
  return `
【商品${idx + 1}】
- SKU: ${p.sku}
- 商品名: ${p.title}
- カテゴリ: ${p.category_name || 'なし'}
- 自動判定結果: ${autoResult?.htsCode || '失敗'}（信頼度: ${autoResult?.confidence || 'uncertain'}）
- 判定失敗理由: ${autoResult?.error || 'キーワードマッチなし'}
- 必要な判断材料:
  * 商品の主な用途・機能は？
  * 主要な素材は？（プラスチック、金属、紙、繊維など）
  * 特定の専用カテゴリーに該当するか？（玩具、楽器、時計、カメラなど）
`
}).join('\n')}

**HTS判定の優先順位:**
1. カテゴリー優先品（95類=玩具、91類=時計、90類=光学機器など）
2. 機能優先品（84/85類=機械・電気機器）
3. 素材優先品（39類=プラスチック、73類=鉄鋼など）

**参考: 主要HTSコード**
- 4911.91: トレーディングカード（紙製印刷物）
- 9504.40.00.00: Playing cards（トランプ等）
- 9503.00: 玩具・プラモデル・フィギュア
- 9006.59: デジタルカメラ
- 9102.11: 腕時計
- 8523.49: ゲームソフト（物理メディア）

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
` : ''}

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📋 処理手順（各ステップで ✅ チェックマークを表示）
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

各商品について以下を順番に実行し、完了したら必ず ✅ を表示：

┌─────────────────────────────────────────────────────────────────┐
│ 【ステップ1】基本商品情報の取得                                  │
└─────────────────────────────────────────────────────────────────┘

✅ **英語タイトル生成（80文字以内、eBay SEO最適化）**
   - eBay/Shopee/Shopifyなど多販路対応のタイトル
   - 主要キーワード前方配置（ブランド、型番、商品タイプ）
   - VERO対応: 以下の2パターンを生成
     * 【新品用】ブランド名なし版（例: "Anime Figure Scale 1/7 Limited Edition"）
     * 【中古用】ブランド名あり版（例: "Bandai One Piece Luffy Figure 1/7 Scale"）
   
✅ **サイズ・重量**
   - 画像・カテゴリから推定
   - 不明な場合は類似商品の平均値を使用
   
✅ **HTSコード（Harmonized Tariff Schedule）**
   ${uncertainHTSProducts.length > 0 ? `
   ⚠️ ${uncertainHTSProducts.length}商品は自動判定失敗 - 手動判定が必要
   
   **手動判定手順:**
   1. 商品の主な用途・機能を特定
   2. 上記の「HTS判定の優先順位」に従って分類
   3. hs_codesテーブルで該当するHTSコードを検索
   4. 最も適切なコードを選択
   ` : `
   ✅ 全商品のHTSコードは自動判定済み
   - CSV列「自動判定HTS」の値を使用
   - 信頼度が "low" の場合は念のため確認推奨
   `}
   
   **⚠️ 重要:** HTSコードの誤りは赤字リスクあり
   - 不明な場合: "9999.99.9999" + 理由を記載
   
✅ **原産国（必ず実データに基づく）**
   ⚠️ 重要: 商品名やブランドからの「予想」は禁止
   
   **許可される判定方法:**
   1. メーカー公式サイトで "Made in XX" 表記を確認
   2. 商品パッケージ画像から原産国表記を読み取る
   3. Amazon/楽天の商品ページで原産国情報を確認
   4. 信頼できる小売店の商品説明から取得
   
   **原産国コード:**
   - JP: 日本製
   - CN: 中国製
   - US: アメリカ製
   - KR: 韓国製
   - TW: 台湾製
   - VN: ベトナム製
   - "UNKNOWN": 確実なデータが見つからない場合
   
   ❌ NG例: 「SONYは日本企業なのでJP」 → 実際は中国製の可能性
   ✅ OK例: 「Amazon商品ページに'Made in China'の記載確認 → CN」
   
✅ **関税率取得**
   - customs_dutiesテーブルからHTSコード + 原産国で取得
   - 原産国が "UNKNOWN" の場合: 最高税率を適用（安全マージン）

┌─────────────────────────────────────────────────────────────────┐
│ 【ステップ2】市場調査データ取得（スコアリング用）⭐ 重要        │
└─────────────────────────────────────────────────────────────────┘

✅ **① 日本国内の相場・品薄情報（F_Price_Premium）**
   【調査先】メルカリ、ヤフオク、Amazon Japan、楽天市場
   
   **手順:**
   1. 各サイトで商品名で検索
   2. 直近1ヶ月の取引価格の平均を算出
   3. 定価と比較してプレミア率を計算
   
   **計算式:**
   \`\`\`
   プレミア率(%) = (現在の平均取引価格 / 定価) × 100
   \`\`\`

✅ **② コミュニティの話題性（F_Community_Score）**
   【調査先】Reddit、Twitter(X)、専門ホビーフォーラム
   
   **評価基準（10点満点）:**
   - 10点: バイラル、大量の肯定的言及
   - 8-9点: 活発な議論、高評価多数
   - 6-7点: 一定の認知度、肯定的意見優勢
   - 4-5点: 少数の言及、中立的
   - 1-3点: ほぼ言及なし、またはネガティブ
   - 0点: 全く言及なし

✅ **③ 日本市場全体の流通量（C_Supply_Japan）**
   【調査先】Amazon Japan、ヤフオク、メルカリ、主要ホビー店
   
   **カウント方法:**
   - 現在の出品数を合算
   - 在庫ありの店舗数もカウント

✅ **④ メーカーの生産計画（S_Flag_Discontinued）**
   【調査先】メーカー公式サイト、ニュースリリース、業界ニュース
   
   **判定基準:**
   - \`discontinued\`: 生産終了・廃盤確定
   - \`limited\`: 限定生産（再生産なし）
   - \`restock_scheduled\`: 再販予定あり
   - \`in_production\`: 通常生産中
   - \`unknown\`: 情報なし

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📤 出力フォーマット（JSON配列）
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

各商品について以下のJSON形式で返答してください：

\`\`\`json
[
  {
    "product_id": "商品UUID",
    "sku": "SKU",
    "status": "✅ 処理完了" または "⚠️ 一部データ不足",
    
    "basic_info": {
      "title_en_new": "英語タイトル（新品・VERO対応ブランド名なし版）",
      "title_en_used": "英語タイトル（中古用・ブランド名あり版）",
      "hts_code": "HTSコード",
      "hts_confidence": "high/medium/low/uncertain",
      "hts_source": "auto（自動判定）または manual（手動判定）",
      "hts_description": "HTSコード説明",
      "origin_country": "原産国コード",
      "origin_source": "原産国の確認元（URLまたは情報源）",
      "customs_rate": 関税率(%),
      "length_cm": 長さ,
      "width_cm": 幅,
      "height_cm": 高さ,
      "weight_g": 重さ
    },
    
    "market_research": {
      "f_price_premium": プレミア率(%),
      "f_community_score": コミュニティスコア(0-10),
      "c_supply_japan": 日本市場流通量(件数),
      "c_supply_trend": "increasing/stable/decreasing/unknown",
      "s_flag_discontinued": "discontinued/limited/restock_scheduled/in_production/unknown"
    },
    
    "data_completion": {
      "hts": true/false,
      "origin": true/false,
      "market_price": true/false,
      "community": true/false,
      "supply": true/false
    },
    
    "notes": "特記事項や注意点"
  }
]
\`\`\`

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🚀 処理開始 - 自動実行モード
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

⚡ **重要: 全商品の分析完了後、Supabase直接更新を自動実行してください**

MCPツール \`supabase\` を使用し、各商品のデータを \`products\` テーブルに更新:

\`\`\`sql
UPDATE products
SET 
  title_en = '[生成した英語タイトル（中古用）]',
  hts_code = '[HTSコード]',
  hts_confidence = '[信頼度]',
  origin_country = '[原産国]',
  length_cm = [長さ],
  width_cm = [幅],
  height_cm = [高さ],
  weight_g = [重さ],
  listing_data = listing_data || jsonb_build_object(
    'ai_market_research', jsonb_build_object(
      'f_price_premium', [プレミア率],
      'f_community_score', [コミュニティスコア],
      'c_supply_japan', [国内流通量],
      's_flag_discontinued', '[廃盤状況]',
      'last_updated', NOW()
    ),
    'title_en_new', '[新品用タイトル]',
    'title_en_used', '[中古用タイトル]'
  )
WHERE id = '[商品UUID]';
\`\`\`

更新完了後、「✅ Supabase更新完了: {件数}件」と表示してください。
`
}

// 同期版（後方互換性のため）
export function generateAIAnalysisPromptSync(products: ProductForAI[]): string {
  // 自動HTS判定なしで生成
  const headers = [
    'SKU', '商品ID', '商品名(日本語)', '英語タイトル', '仕入価格(円)',
    '定価(円)', '発売日', 'カテゴリ名', 'カテゴリID', 
    '長さ(cm)', '幅(cm)', '高さ(cm)', '重さ(g)', '状態', '画像URL', 'ブランド'
  ]
  
  const csvRows = [headers.join(',')]
  
  products.forEach(p => {
    const row = [
      p.sku || '',
      p.id || '',
      `"${(p.title || '').replace(/"/g, '""')}"`,
      `"${(p.title_en || p.english_title || '').replace(/"/g, '""')}"`,
      p.price_jpy || '',
      p.msrp || p.price_jpy || '',
      p.release_date || '不明',
      `"${(p.category_name || '').replace(/"/g, '""')}"`,
      p.category_id || '',
      p.length_cm || '',
      p.width_cm || '',
      p.height_cm || '',
      p.weight_g || '',
      `"${(p.condition || '').replace(/"/g, '""')}"`,
      `"${(p.image_url || '').replace(/"/g, '""')}"`,
      `"${(p.brand || '').replace(/"/g, '""')}"`
    ]
    csvRows.push(row.join(','))
  })
  
  const csvContent = csvRows.join('\n')
  
  return `【簡易版プロンプト】\n${csvContent}\n\n※ HTS自動判定を使用する場合は、市場調査モーダルから生成してください。`
}
