...
import { Lightbulb, BookOpen, Clock, Zap, Sun, Moon, CheckCircle, Trash2, Calendar, Menu, Activity, Brain } from 'lucide-react'; // アイコンライブラリの利用
...
const DUMMY_RECIPES = [
  { id: 'r1', name: '高タンパク鶏むね肉グリル (国産)', nutrients: { protein: 45, iron: 2, calcium: 50, sodium: 300, sugar: 5 }, advParams: { hasAdditives: 'なし', foodPairing: 'ブロッコリー' } },
  { id: 'r3', name: '輸入フルーツの砂糖漬けヨーグルト', nutrients: { protein: 15, iron: 0.5, calcium: 250, sodium: 50, sugar: 65 }, advParams: { hasAdditives: '着色料', foodPairing: 'なし' } },
  { id: 'r4', name: '有機野菜とツナのサラダ', nutrients: { protein: 20, iron: 1, calcium: 30, sodium: 200, sugar: 5 }, advParams: { hasAdditives: 'なし', foodPairing: '穀物' } },
];
const EXERCISE_TYPES = [
  { id: 'e1', name: '🧘 ヨガ (30分)', target: '柔軟性・リラックス', calories: 150 },
  { id: 'e2', name: '🚶 散歩 (40分/有酸素)', target: '有酸素運動・習慣', calories: 200 },
  { id: 'e3', name: '🏃 HIIT (30分/高負荷)', target: '脂肪燃焼', calories: 400 },
  { id: 'e4', name: '💪 筋トレ (全身)', target: '筋力UP・基礎代謝', calories: 300 },
];
const DUMMY_USER_STATS = { bodyFat: 28, targetBodyFat: 20, stressLevel: 75 }; // ストレスレベルを追加 (0-100)

// --- 新しい定数: ウェルビーイングタスク ---
const WELLBEING_TYPES = [
    { id: 'w1', name: '🧠 クンルン (20分)', type: 'mental', icon: <Brain className="w-4 h-4" />, target: '意識活性化' },
    { id: 'w2', name: '🧘 瞑想/内観 (15分)', type: 'mental', icon: <Lightbulb className="w-4 h-4" />, target: 'ストレス軽減' },
    { id: 'w3', name: '📝 日記/夢日記', type: 'mental', icon: <BookOpen className="w-4 h-4" />, target: '自己認識' },
    { id: 'w4', name: '💊 noosサプリ摂取', type: 'sleep', icon: <Zap className="w-4 h-4" />, target: '睡眠の質' },
    { id: 'w5', name: '⏰ 自然光タイマー起床', type: 'sleep', icon: <Sun className="w-4 h-4" />, target: '体内時計リセット' },
    { id: 'w6', name: '😴 アラートなしで起床', type: 'sleep', icon: <Moon className="w-4 h-4" />, target: '睡眠効率' },
    { id: 'w7', name: '🧘‍♂️ チベット式健康法 (10分)', type: 'mental', icon: <Activity className="w-4 h-4" />, target: 'エネルギー調和' },
];
...
const HEALTH_RISKS = [
  ...
  { name: '慢性ストレス', targetNutrient: 'N/A', prevention: '瞑想、クンルン、睡眠効率向上', medical: 'コルチゾール値検査' },
];
...
// 体脂肪率に基づいた運動メニュー提案ロジック（モック）
const getRecommendedExercise = (date) => {
  const day = date.getDay();
  const bodyFat = DUMMY_USER_STATS.bodyFat;

  if (bodyFat >= 25) {
    if (day % 3 === 1) return { recipeId: 'e3', time: '朝', description: '体脂肪燃焼のため、HIITを優先' }; 
    if (day % 3 === 2) return { recipeId: 'e4', time: '夜', description: '基礎代謝アップのため、筋トレを優先' }; 
    return { recipeId: 'e2', time: '夕', description: '軽い有酸素運動で継続性を重視' }; 
  } else {
    if (day % 2 === 0) return { recipeId: 'e1', time: '朝', description: '柔軟性とリラックスを重視' };
    return { recipeId: 'e4', time: '夜', description: 'シェイプアップのため筋トレ' };
  }
};

// ストレスレベルに基づいたウェルビーイングメニュー提案ロジック（モック）
const getRecommendedWellbeing = (stressLevel) => {
    if (stressLevel >= 70) {
        // 高ストレス時: 瞑想と睡眠効率を重視
        return [
            { id: 'w2', description: '高ストレスのため、瞑想で心を鎮めます' },
            { id: 'w4', description: '質の高い睡眠のため、サプリ摂取' }
        ];
    } else if (stressLevel >= 50) {
        // 中ストレス時: 整理とリセットを重視
        return [
            { id: 'w3', description: '日記で思考を整理します' },
            { id: 'w7', description: 'チベット式健康法でエネルギー調和' }
        ];
    } else {
        // 低ストレス時: 活性化と向上を重視
        return [
            { id: 'w1', description: '意識活性化のためクンルン' },
            { id: 'w5', description: '体内時計リセットのため自然光起床' }
        ];
    }
};
...
const PreventionView = () => (
    <div className="p-8 bg-white rounded-xl shadow-inner min-h-[70vh]">
        ...
        <p className="text-gray-600 mb-8">
            あなたの健康リスクに基づき、栄養、運動、医療の側面から統合的な予防策を提案します。現在のストレスレベルは **{DUMMY_USER_STATS.stressLevel}%** です。
        </p>
                ...
                <div key={index} className="p-5 border border-red-300 bg-red-50 rounded-lg shadow-md hover:shadow-xl transition duration-300">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-3 text-sm">
                        <div className="bg-yellow-100 p-3 rounded-md">
                            <p className="font-semibold text-gray-800 border-b pb-1 mb-1">栄養</p>
                            <p className="text-gray-600">制限/強化栄養素: **{risk.targetNutrient}**</p>
                        </div>
                        <div className="bg-blue-100 p-3 rounded-md">
                            <p className="font-semibold text-gray-800 border-b pb-1 mb-1">運動</p>
                            <p className="text-gray-600">対策: **{risk.prevention.split(', ')[1] || 'N/A'}**</p>
                        </div>
                        <div className="bg-purple-100 p-3 rounded-md">
                            <p className="font-semibold text-gray-800 border-b pb-1 mb-1">精神・睡眠</p>
                            <p className="text-gray-600">対策: **{risk.prevention.split(', ').slice(2).join(', ') || 'N/A'}**</p>
                        </div>
                        <div className="bg-green-100 p-3 rounded-md">
                            <p className="font-semibold text-gray-800 border-b pb-1 mb-1">医療</p>
...
const App = () => {
  ...
  const [wellbeingPlans, setWellbeingPlans] = useState([]); // 新しいウェルビーイングデータ
  ...
  const handleAddMeal = useCallback(async (dateStr, recipeId) => {
    ...
    } catch (e) { console.error('献立追加エラー:', e); }
  ...
  const handleAddExercise = useCallback(async (dateStr, exerciseId, isRecommended = false) => {
    ...
    } catch (e) { console.error('運動追加エラー:', e); }
  }, [userId]);
  ...
  const handleAddWellbeing = useCallback(async (dateStr, wellbeingId, isRecommended = false) => {
    ...
    } catch (e) { console.error('ウェルビーイングタスク追加エラー:', e); }
  }, [userId]);
  ...
  const handleToggleCompletion = useCallback(async (itemId, collectionType) => {
    ...
    } catch (e) { console.error('実績更新エラー:', e); }
  ...
  const handleDeleteItem = useCallback(async (itemId, collectionType) => {
    ...
    } catch (e) { console.error('データ削除エラー:', e); }
  }, [userId]);
  ...
  const GoalBadge = ({ date, plans, collectionType }) => {
    ...
    if (completed === total) {
      return <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${color}`}>🏆 完了</span>;
    }
    
    if (completed > 0 && completed < total) {
        return <span className={`text-xs font-bold px-2 py-0.5 rounded-full bg-yellow-100 text-yellow-700`}>{completed}/{total} 進行中</span>;
    }
        ...
        <div className="p-6">
            <div className="flex justify-between items-center mb-6 border-b pb-3">
                ...
                <div className="flex space-x-3 items-center">
                    ...
                    >
                        <Calendar className="w-4 h-4 inline mr-1"/> Googleカレンダーと同期
                    </button>
                    <button 
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-2 absolute bottom-2 left-2 right-2">
                      ...
                      >
                          <option value="" disabled>+ {collectionType === 'menu' ? '献立' : collectionType === 'exercise' ? '運動' : 'タスク'}を追加...</option>
  ...
  const MealItem = ({ item, onToggle, onDelete }) => (
    <div className="bg-gray-100 p-2 rounded-lg shadow-sm border border-gray-200">
        <div className="flex justify-between items-start">
            ...
            <button onClick={onDelete} className="text-red-400 hover:text-red-600 p-0.5 rounded-full transition" title="削除">
        ...
        <div className="flex justify-between items-center mt-1">
            <label className="flex items-center cursor-pointer">
                ...
                <span className={`ml-2 text-xs ${item.isCompleted ? 'line-through text-gray-400' : 'text-gray-700'}`}>摂取完了</span>
            </label>
        </div>
    </div>
  );

  // --- 運動関連コンポーネント (Exercise) ---
    ...
    <div className={`p-2 rounded-lg shadow-sm border ${item.isRecommended ? 'bg-yellow-100 border-yellow-300' : 'bg-blue-100 border-blue-300'}`}>
        <div className="flex justify-between items-start">
            <p className="text-sm font-semibold text-gray-800 leading-tight">
                {item.exerciseName}
                {item.isRecommended && <span className="text-xs bg-yellow-400 text-gray-800 px-1 ml-1 rounded-full font-bold">推奨</span>}
            </p>
            <button onClick={onDelete} className="text-red-400 hover:text-red-600 p-0.5 rounded-full transition" title="削除">
                <Trash2 className="h-4 w-4"/>
            </button>
        ...
        <div className="flex justify-between items-center mt-1">
            <label className="flex items-center cursor-pointer">
                ...
                <span className={`ml-2 text-xs ${item.isCompleted ? 'line-through text-gray-400' : 'text-gray-700'}`}>実行完了</span>
            </label>
        ...
        <div className={`p-2 rounded-lg shadow-sm border ${bgColor}`}>
            ...
            <div className="flex justify-between items-center mt-1">
                <label className="flex items-center cursor-pointer">
                    ...
                    <span className={`ml-2 text-xs ${item.isCompleted ? 'line-through text-gray-400' : 'text-gray-700'}`}>完了</span>
                </label>
  ...
  // ウェルビーイングメニュー提案コンポーネント (Recommendation)
        ...
        if (!hasRecommendation && recommendations.length > 0) {
            return (
                <button
                    ...
                    title={`現在のストレスレベル(${DUMMY_USER_STATS.stressLevel}%)に基づいた推奨タスクを追加`}
                >
                    本日のおすすめ追加
                </button>
            );
  ...
  const WellbeingCalendarView = () => (
    <BaseCalendar
      ...
      title="ウェルビーイングカレンダー (精神・睡眠)"
      collectionType="wellbeing"
      syncMock="Googleカレンダー同期: 精神・睡眠タスクを同期するAPIを呼び出します"
    />
  );
      ...
      <header className="bg-white shadow-md">
        <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex justify-between items-center">
          <h1 className="text-2xl font-extrabold text-indigo-700 flex items-center">
            <CheckCircle className="w-6 h-6 mr-2 text-green-500"/> 統合予防医療プラットフォーム
          </h1>
          <nav className="flex space-x-3">
            ...
            >
              <Menu className="w-4 h-4 mr-1"/> 献立カレンダー
            </button>
            <button
            ...
            >
              <Activity className="w-4 h-4 mr-1"/> 運動カレンダー
            </button>
            <button
            ...
            >
              <Brain className="w-4 h-4 mr-1"/> ウェルビーイング
            </button>
            <button
            ...
            >
              <Lightbulb className="w-4 h-4 mr-1"/> ダッシュボード
            </button>
          </nav>
