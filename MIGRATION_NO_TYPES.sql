...
-- ポリシー削除（既存の場合）
DROP POLICY IF EXISTS "全ユーザーがアクセス可能" ON inventory_master;
DROP POLICY IF EXISTS "全ユーザーがアクセス可能" ON set_components;
DROP POLICY IF EXISTS "全ユーザーがアクセス可能" ON inventory_changes;

-- 全ユーザーがアクセス可能（認証なしでも動作）
CREATE POLICY "全ユーザーがアクセス可能" ON inventory_master FOR ALL USING (true);
CREATE POLICY "全ユーザーがアクセス可能" ON set_components FOR ALL USING (true);
CREATE POLICY "全ユーザーがアクセス可能" ON inventory_changes FOR ALL USING (true);
...
DROP TRIGGER IF EXISTS update_inventory_master_updated_at ON inventory_master;
